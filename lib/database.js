...
function Database(filenameGiven, options) {
	...
	const vfs = 'vfs' in options ? options.vfs : 'unix';
	const memvfs = vfs === 'memvfs';

	let filename = filenameGiven.trim();
	if (filename.toLowerCase().startsWith('file:') && !memvfs) throw new TypeError('URI filenames are reserved for internal use only');
	if ('readOnly' in options) throw new TypeError('Misspelled option "readOnly" should be "readonly"');

	const anonymous = filename === '' || filename === ':memory:' || memvfs;
	const memory = util.getBooleanOption(options, 'memory') || memvfs;
	...
	return new CPPDatabase(filename, filenameGiven, memory || anonymous, readonly, fileMustExist, timeout, verbose || null, vfs);
}
