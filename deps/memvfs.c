...
static int memWrite(
  ...
  MemFile *p = (MemFile *)pFile;
  if( iOfst+iAmt>p->sz ){
    printf("iOfst+iAmt>p->sz\n");
    if( iOfst+iAmt>p->szMax ) {     
      printf("iOfst+iAmt>p->szMax\n");
      return SQLITE_FULL;
    }
...
static int memFileControl(sqlite3_file *pFile, int op, void *pArg){
  ...
  if( op==SQLITE_FCNTL_VFSNAME ){
    *(char**)pArg = sqlite3_mprintf("mem(%p,%lld)", p->aData, p->sz);
    rc = SQLITE_OK;
  } else if (op == SQLITE_FCNTL_FILE_POINTER) {
    pArg = pFile;
    rc = SQLITE_OK;
  }
  return rc;
...
static void memvfs_dbsize(sqlite3_context* context, int argc, sqlite3_value** argv) {
    sqlite3 *db = sqlite3_context_db_handle(context);
    MemFile *pFile;
    sqlite3_int64 size = 0;

    if (sqlite3_file_control(db, "main", SQLITE_FCNTL_FILE_POINTER, &pFile) != SQLITE_OK) {
      return;
    }

    if (memFileSize((sqlite3_file *)pFile, &size) != SQLITE_OK) {
      return;
    }
...
#endif
  int rcf = sqlite3_create_function(db, "memvfs_dbsize", 0, SQLITE_UTF8, 0, memvfs_dbsize, 0, 0);
  printf("sqlite3_create_function memvfs_dbsize rc=%d\n", rcf);
